...
<stylesheet xmlns="http://www.w3.org/1999/XSL/Transform" 
	...
	<variable name="allStates" 
		select="//*[(self::s:state or self::s:parallel or self::s:final or self::s:initial or self::s:scxml or self::s:history)]"/>
	<variable name="states" select="//s:state"/>
	<variable name="transitions" select="//s:transition"/>
	
	<variable name="basicStates" 
		select="$allStates[not(.//*[(self::s:state or self::s:parallel or self::s:final or self::s:initial or self::s:scxml or self::s:history)])]"/>
	...
	<variable name="allEventsEnum" select="/s:scxml/c:allEventsEnum/c:event"/>
	<variable name="enumeratedEventsEnum" select="/s:scxml/c:enumeratedEventsEnum/c:event"/>

	<!-- Variables we don't use, but which might be useful at some point -->
	<!--
	...
	<variable name="basicStateDescendantOfParallelStateHash" select="'$basicStateDescendantsOfParallelStates'"/>
	...
	<variable name="genListenerHookRegistrationFunctions">
		var listeners = [];
