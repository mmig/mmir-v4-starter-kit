...
define(['appUtil', 'jsonlint', 'esprima', 'grammarValidator'], function(util, jsonparser, jsparser, GrammarValidator){
	...
	function _createValidator(view, editor, errorMarkerId, warningMarkerId, infoMarkerId){
		...
		/**
		 * HELPER for processing Error generated by ext-json-parser:
		 *        creates error-marker/-messages adds them to the editor
		 */
		var __processExtJsonError = function(err){
...
//							}).join('\n') + '\n--------------\n');
				...
				try {
					...
					result = jsonparser.parse(editor.val());
