...
var processTemplate = function(err, templateStr, context, outputFile){
...
var startProcessing = function(templateFile, targetFile, includeFiles, config){
	...
	var checkProcTemplate = function(){
		...
		if(isLoopCompleted === true && loaded === count){
			...
			startProcessingTemplate(templateFile, targetFile, config);
		}
	};
...
var startProcessingTemplate = function(templateFile, targetFile, config){

	fs.readFile(templateFile, 'utf8', function(error, output) {
		processTemplate(error,output,config,targetFile);
	});
