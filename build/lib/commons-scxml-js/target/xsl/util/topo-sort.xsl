...
--><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:c="http://commons.apache.org/scxml-js" xmlns:exsl="http://exslt.org/common" version="1.0">
	...
	<xsl:template match="/">
		...
		<xsl:variable name="unique-dependency-graph-node-set" select="$dependency-graph-node-set/c:stylesheet[not(@path = preceding-sibling::c:stylesheet/@path)]"/>
	...
	<xsl:template name="topo-sort">
		...
		<xsl:if test="count($next-processed-nodes) &lt; count($dependency-graph-node-set)">
			...
			<xsl:variable name="next-nodes-without-dependencies" select="$dependency-graph-node-set[       not($next-processed-nodes/@path = @path)        and count(c:dependency)        = count(c:dependency[$next-processed-nodes/@path = @path])]"/>
