...
--><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="xsl:import">
		...
		<!-- filter templates -->
		<xsl:variable name="filteredNewTemplates" select="$newTopLevelElements[self::xsl:template]        [not(@name = $elementsAlreadyDefined[self::xsl:template]/@name)]        [not(@match = $elementsAlreadyDefined[self::xsl:template]/@match)]"/>


		<!-- params -->
		<xsl:variable name="filteredNewParams" select="$newTopLevelElements[self::xsl:param]       [not( @name = $elementsAlreadyDefined[self::xsl:param]/@name )]"/>

		
		<!-- output -->
		<xsl:variable name="filteredNewOutput" select="$newTopLevelElements[self::xsl:output]       [not( $elementsAlreadyDefined[self::xsl:output] )]"/>

		<!-- variable -->
		<xsl:variable name="filteredNewVariables" select="$newTopLevelElements[self::xsl:variable]       [not( @name = $elementsAlreadyDefined[self::xsl:variable]/@name )]"/>
