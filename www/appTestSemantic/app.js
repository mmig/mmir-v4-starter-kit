...
define(['require', 'jquery', 'mainView', 'appUtil'
        , 'mmirf/encodeUtils'
        , 'mmirf/constants', 'mmirf/commonUtils', 'mmirf/languageManager', 'mmirf/configurationManager', 'mmirf/semanticInterpreter', 'mmirf/grammarConverter'
        , 'mmirf/checksumUtils', 'mmirf/util/extend'
...
//			$('#grammar-id').val(currentLangSelection);
		...
		semanticInterperter = require('mmirf/semanticInterpreter');
		/**
...
//			}
	...
	function processInterpretation(view, viewModel) {
		...
		var res, res2;
		try{
			res = semanticInterpreter.interpret(asr_result.toLowerCase(), viewModel.id);
		} catch (err){
		...
		if (isEnableAlternateStopwordProcessing) {
			try{
				res2 = semanticInterpreter.interpret_alt(asr_result.toLowerCase(), viewModel.id);//FIXME
			} catch (err2){
	...
	function benchmarkInterpretation(view, viewModel) {
		...
		if (isEnableAlternateStopwordProcessing) {
			try{
				res2 = semanticInterpreter.interpret_alt(asr_result.toLowerCase(), viewModel.id);
			} catch (err){
...
//		clearInterpret(view);
		...
		var doStartBenchmark = function() {
			...
			for (var i = 0; i < iterations; ++i) {
				semanticInterpreter.interpret(asr_result.toLowerCase(), viewModel.id);
			}
			...
			if (isEnableAlternateStopwordProcessing) {
				...
				for (var i = 0; i < iterations; ++i) {
					semanticInterpreter.interpret_alt(asr_result.toLowerCase(), viewModel.id);
				}
	...
	function _convertJsonTo(view, viewModel, convertFuncName){
		...
		if(convertFuncName === 'encodeUmlauts' || convertFuncName === 'decodeUmlauts'){
			//-> these need to be mapped to the generic recoding-function of GrammarConverter
			//set the specific recoding-function (used by the generic JSON-recoder):
			recodingFunc = encodeUtils[convertFuncName];
