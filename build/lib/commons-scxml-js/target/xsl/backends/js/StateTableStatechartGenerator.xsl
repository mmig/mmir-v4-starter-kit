...
--><stylesheet xmlns="http://www.w3.org/1999/XSL/Transform" xmlns:s="http://www.w3.org/2005/07/scxml" xmlns:c="http://commons.apache.org/scxml-js" version="1.0">
	...
	<output method="text"/><param name="log" select="false()"/><param name="genListenerHooks" select="true()"/><param name="noIndexOf" select="false()"/><param name="classBaseName" select="'StatechartExecutionContext'"/><variable name="allStates" select="//*[(self::s:state or self::s:parallel or self::s:final or self::s:initial or self::s:scxml or self::s:history)]"/><variable name="states" select="//s:state"/><variable name="transitions" select="//s:transition"/><variable name="basicStates" select="$allStates[not(.//*[(self::s:state or self::s:parallel or self::s:final or self::s:initial or self::s:scxml or self::s:history)])]"/><variable name="compositeStates" select="$allStates[.//*[(self::s:state or self::s:parallel or self::s:final or self::s:initial or self::s:scxml or self::s:history)]]"/><variable name="allEventsEnum" select="/s:scxml/c:allEventsEnum/c:event"/><variable name="enumeratedEventsEnum" select="/s:scxml/c:enumeratedEventsEnum/c:event"/><variable name="abstractStateName" select="'AbstractState'"/><variable name="delayedSendBaseVariableName" select="'_$timeoutId'"/><variable name="basicStateDescendantOfParallelStateHash" select="'$basicStateDescendantsOfParallelStates'"/><variable name="defaultIteratorVarName" select="concat(generate-id(),'_iterator')"/><variable name="defaultHoistVarName" select="concat(generate-id(),'_hoist')"/><template name="genForEach">
		<param name="var"/>
		<param name="in"/>
		<param name="when"/>
		<param name="do"/>
		...
		function <value-of select="@name"/><value-of select="$classBaseName"/>(){
				...
				<call-template name="genParallelStateBasicSubstatesHash"/>
	...
	</template><template name="genParallelStateBasicSubstatesHash">
		var <value-of select="$basicStateDescendantOfParallelStateHash"/> = {
			<for-each select="//s:parallel">
				<value-of select="@id"/> : {
					<for-each select=".//*[@c:isBasic]">
						<call-template name="genStateRefIdentifier">
	...
	<!--TODO-->
		...
		<!--<variable name="isChildOfParallel" select="$s/../self::s:parallel"/>-->
		...
		if(<value-of select="$historyStateReference"/>.lastConfiguration){
			...
			return {
				preemptedBasicStates : <call-template name="genPreemptedBasicStateReference"><with-param name="t" select="$t"/></call-template>,
				action: function(){
					//transition action
					<if test="$log">
						<choose>
	...
	</template><template name="genPreemptedBasicStateReference">
		<param name="t"/>

		<choose>
			<when test="$t/c:nearestParallelDescendantOfLCA">
				<value-of select="$basicStateDescendantOfParallelStateHash"/>["<value-of select="$t/c:nearestParallelDescendantOfLCA"/>"]
			</when>
			<otherwise> {} </otherwise>
		</choose>
	...
	</variable><variable name="defaultEventLiteral" select="'$default'"/><variable name="inPredicateFunctionStateReference" select="'state.id'"/><variable name="inPredicateFunctionStateIdReference" select="'STATE_INT_ID_TO_OBJECT_MAP[s]'"/><variable name="genHistoryTriggerDispatcherInnerForEachStateReference" select="'STATE_INT_ID_TO_OBJECT_MAP[state]'"/><variable name="eventToNameMap" select="'TRIGGER_ID_TO_NAME_MAP[e]'"/><variable name="prefixEventDispatchInvocation" select="'$dispatchPrefixEvent(state,e)'"/><variable name="currentConfigurationExpression">
	...
	</variable><variable name="genHistoryTriggerDispatcherCurrentConfigurationAssignmentRHS">
		<call-template name="genMap">
	...
	</variable><variable name="genNonBasicTriggerDispatcherExitBlockIteratorExpression">
		<call-template name="genMap">
	...
	</template><template name="genPrefixDispatchHandlerFunction">
	...
	</template><template name="genPrefixDispatchHandlerSwitchyard">
	...
	</template><template name="genPrefixDispatchHandlerStateCase">
		<param name="s"/>
	...
	</template><template name="genTriggerHandlerFunctions">
	...
	</template><template name="genNonParallelSubstateConfigurationSetString">
		<param name="t"/>
	...
	</template><template name="genParallelSubstateAndCompositeConfigurationSetString">
		<param name="s"/>
		<param name="t"/>
		...
		<variable name="initialStateName" select="$allStates[@id = /s:scxml/s:initial/s:transition/c:targets/c:target/c:targetState]/@c:enumId"/>
	...
	</template><template name="genHistoryTriggerDispatcherHistoryStateReference">
		<param name="s"/>

		<value-of select="$s/@id"/>
	</template><template name="genExternalTriggerDispatcherRunToCompletionEventValue">
		<param name="eventName"/>
	...
	</template><template name="genStateRefIdentifier">
		<param name="s"/>
		<value-of select="$s/@c:stateNum - 1"/>
	</template><template name="genStateHooks"/><template name="genTriggerIntEnum"/>
			...
			<for-each select="$basicStates">
				...
				<text>[</text>
					...
					<for-each select="$enumeratedEventsEnum">
						...
						<variable name="correspondingTransitions" select="$currentState/c:enumeratedEventTransitions/c:enumeratedTransition[$isDefaultTransition][not(@event)] |          $currentState/c:enumeratedEventTransitions/c:enumeratedTransition[not($isDefaultTransition)][@event = $currentEvent/c:name]"/>
